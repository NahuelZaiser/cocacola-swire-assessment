...
export function Stepper({ current }: StepperProps) {
  ...
  return (
    <div className="border-b border-red-100 bg-white/90 backdrop-blur-md shadow-[0_1px_0_rgba(230,27,43,0.06)]">
                ...
                <div className="flex flex-col items-center gap-1.5 min-w-0 w-full">
                  <div
                    className={`
                      ...
                      ${isActive ? 'bg-red-50 border-[#E61B2B] text-[#B80D1A] shadow-[0_0_0_4px_rgba(230,27,43,0.10)] scale-[1.03]' : ''}
                      ${!isCompleted && !isActive ? 'bg-slate-100 border-slate-200 text-slate-500' : ''}
                    `}
                  >
                  ...
                  <span
                    className={`text-[10px] sm:text-xs font-semibold truncate ${
                      isActive ? 'text-[#B80D1A]' : isCompleted ? 'text-emerald-600' : 'text-slate-500'
                    }`}
                  >
                    {step.shortLabel}
                  </span>
